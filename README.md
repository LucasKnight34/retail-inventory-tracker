...
This project was built intentionally to demonstrate full-stack engineering fundamentals in a way that mirrors real enterprise patterns. Using raw SQL via `node-postgres` rather than an ORM meant writing and reasoning about actual queries — JOINs, aggregates, indexes, and sequence management — rather than abstracting them away. Implementing JWT authentication from scratch reinforced how token-based auth works at the HTTP layer, including the security decision to return identical error messages for wrong email versus wrong password to prevent user enumeration. Setting up GitHub Actions taught me how CI environments differ from local ones (no pre-existing database, SSL requirements on cloud Postgres), and debugging those failures sharpened my ability to read logs and isolate root causes systematically. Deploying a split frontend/backend architecture to Render demonstrated how environment variables, CORS, and SSL configuration differ between development and production contexts.
